    ...
    <?php $count = 0 ?>
        @foreach($posts->sortByDesc('view_count') as $post)
            ...
            <a href="{{route('posts.show',['post'=>$post])}}" class="inline-block min-h-full min-w-max max-w-sm mx-4 my-3 p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                ...
                <h5 class="mb-2 max-w-sm break-words text-2xl font-bold tracking-tight text-gray-900 dark:text-white ">
                    {{$post->user->name}}
                </h5>
                <h5 class="mb-2 max-w-sm break-words text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                    {{$post->title}}
                </h5>
                <p class="font-normal text-gray-700 dark:text-gray-400 max-w-sm break-words">
                    {{$post->description}}
                </p>
            </a>
            <?php $count =$count+1 ?>
    ...
    <div class="flex justify-center flex-wrap ">
        ...
        @if($count < 5)
            <a href="{{route('posts.show',['post'=>$post])}}" class="post-block">
                ...
                <p class="font-normal max-w-sm break-words text-gray-700 dark:text-gray-400">
                    {{$post->description}}
                </p>
            </a>
            <?php $count =$count+1 ?>
